...
public class InputField extends Gui {
    ...
    private int color = new Color(170, 170, 170).getRGB();
    ...
    public InputField(int x, int y, int width, int height, boolean hideText, String defaultText, String textPrefix, int maximumLength, InputField.InputFlavor inputFlavor) {
        this.text = "";
        this.focused = false;
        this.textColor = new Color(170, 170, 170).getRGB();
        ...
        if (this.isVisible()) {
            ...
            if (this.isFocused() && System.currentTimeMillis() % 1060L >= 530L) {
                if (this.cursorPosition == this.text.length()) {
                    ...
                    Gui.drawRect(this.x + 5 + i1, this.y + 2, this.x + 5 + i1 + 1, this.y + this.height - 11, -570425345);
                }
            }

            if (this.textSelected) {
                Gui.drawRect(this.x + 5, this.y, this.x + 5 + k, this.y + this.height - 10, new Color(51, 144, 255).getRGB());
    ...
    public static class InputFlavor {
        ...
        public static final InputField.InputFlavor NORMAL = new InputField.InputFlavor("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!#$%§&\'*+-/=?^_`{|}~;.@\"(),:;<>@[\\] ");
